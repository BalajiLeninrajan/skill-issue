...
// .wrangler/tmp/bundle-KPAwjc/checked-fetch.js
...
// .wrangler/tmp/bundle-KPAwjc/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-KPAwjc/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
init_checked_fetch();
init_modules_watch_stub();
async function insertHope(db, id, email, companyName, jobTitle, scheduledAt) {
  const now = Math.floor(Date.now() / 1e3);
  await db.prepare(
    `INSERT INTO hope_registrations (id, email, company_name, job_title, scheduled_at, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`
  ).bind(id, email, companyName, jobTitle, scheduledAt, now).run();
}
__name(insertHope, "insertHope");
async function getPendingRejections(db) {
  const now = Math.floor(Date.now() / 1e3);
...
__name(getPendingRejections, "getPendingRejections");
async function markAsSent(db, id) {
  const now = Math.floor(Date.now() / 1e3);
  await db.prepare(`UPDATE hope_registrations SET sent_at = ? WHERE id = ?`).bind(now, id).run();
}
__name(markAsSent, "markAsSent");
...
__name(getRandomTemplate, "getRandomTemplate");
function renderTemplate(template, companyName, jobTitle) {
  const render2 = /* @__PURE__ */ __name((text) => text.replace(/\{\{company_name\}\}/g, companyName).replace(/\{\{job_title\}\}/g, jobTitle), "render");
  return {
    subject: render2(template.subject),
    body: render2(template.body)
  };
}
__name(renderTemplate, "renderTemplate");

// src/email.ts
async function sendRejectionEmail(resendApiKey, email, companyName, jobTitle) {
...
__name(sendRejectionEmail, "sendRejectionEmail");
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/register-hope" && request.method === "POST") {
      try {
        ...
        const id = generateId();
        const scheduledAt = Math.floor(Date.now() / 1e3) + getRandomDelay();
